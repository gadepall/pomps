...
\begin{enumerate}[label=\thesubsection.\arabic*.,ref=\thesubsection.\theenumi]
	\item The roots of the equation are known as the eigenvalues of $\bm{\Sigma}$.
\begin{align}
	\mydet{\lambda \vec{I}-\bm{\Sigma}} = 0
\end{align}
...
%% 
\text{and substituting } \eqref{eq:conic_parmas_c_def}
\text{in } \eqref{eq:conic_simp}
...
\item
			In 
			\eqref{eq:conic_affine},
	substituting $\vec{y} = \vec{0}$, the center/vertex for the quadratic form is obtained as
    \begin{align}
	    \vec{x} = \vec{c}, 
    \end{align}
